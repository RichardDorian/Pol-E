...
class SensorsData:
    ...
    middle_is_object_detected = False
...
def SensorsThread():
    ...
    other_sensors_interval = const(2000)

    while True:
        SensorsData.left_is_object_detected = left_sensor.is_object_detected()
        SensorsData.middle_is_object_detected = middle_sensor.is_object_detected()

        SensorsData.left_distance = left_sensor.get_measured_distance()
        SensorsData.middle_distance = middle_sensor.get_measured_distance()

        if (last_data_dump_time + other_sensors_interval) <= get_time_ms():
            last_data_dump_time = get_time_ms()
